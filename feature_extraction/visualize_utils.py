...
def get_top_percent_farthest_by_tracer(distance_df, top_percent=0.05):
    ...
    for tracer, tracer_df in distance_df.groupby('tracer', sort=False):
        tracer_df.drop_duplicates(inplace=True)
        print(f"Processing tracer: {tracer} with {len(tracer_df)} unlabelled samples")
        # HARDCODING THE NUMBER OF UNLABELLED SAMPLES FOR EACH TRACER
        if tracer == "PSMA": 
            total_number = 379
        else: 
            total_number = 664
        ...
        top_n = math.ceil(total_number * top_percent)
