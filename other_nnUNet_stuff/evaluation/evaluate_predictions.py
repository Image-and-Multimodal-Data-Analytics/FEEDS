...
def con_comp_fast(seg_array, connectivity=26, min_size=0):
    # 1) ensure binary, compact, contiguous
    seg = np.asarray(seg_array != 0, dtype=np.uint8)
    ...
    # 3) crop to foreground bbox
    nz = np.argwhere(seg)
    z0, y0, x0 = nz.min(0)
    z1, y1, x1 = nz.max(0) + 1
    crop = seg[z0:z1, y0:y1, x0:x1]

    # 4) connected components (smaller dtype)
    labels_crop = cc3d.connected_components(
        crop, connectivity=connectivity, out_dtype=np.uint32
    )

    # 5) (optional) remove tiny components to avoid huge label counts
    if min_size > 0:
        # relabel with size filtering
        counts = np.bincount(labels_crop.ravel())
        small = np.where(counts < min_size)[0]
        if len(small) > 1 or (len(small)==1 and small[0] != 0):
            mask_small = np.isin(labels_crop, small)
            labels_crop[mask_small] = 0
            # relabel after removing small components
            labels_crop = cc3d.connected_components(
                labels_crop != 0, connectivity=connectivity, out_dtype=np.uint32
            )

    # 6) paste back
    out = np.zeros_like(seg, dtype=np.uint32)
    out[z0:z1, y0:y1, x0:x1] = labels_crop
    return out
...
def false_pos_pix(gt_array,pred_array, reference_file):
    # compute number of voxels of false positive connected components in prediction mask
    print('I am now computing connected components of pred for ' + reference_file,flush=True) 
    pred_conn_comp = con_comp(pred_array)
    #pred_conn_comp = con_comp_fast(pred_array)

    print('I am done computing connected components of pred for ' + reference_file,flush=True) 
    counts = np.bincount(pred_conn_comp.ravel())
    print(counts)
    if counts.size == 0:
        return 0

    # labels that overlap GT (exclude 0)
    overlap_labels = np.unique(pred_conn_comp[gt_array != 0])
    print(overlap_labels.shape)
    overlap_labels = overlap_labels[overlap_labels != 0]
    print(overlap_labels.shape)

    # all predicted foreground voxels minus overlapping components' voxels
    total_pred_vox = int(counts[1:].sum())
    overlapping_vox = int(counts[overlap_labels].sum()) if overlap_labels.size else 0
    print(total_pred_vox - overlapping_vox)
    return total_pred_vox - overlapping_vox
...
def false_neg_pix(gt_array,pred_array, reference_file):
    # compute number of voxels of false negative connected components (of the ground truth mask) in the prediction mask
    gt_conn_comp = con_comp(gt_array)
    #gt_conn_comp = con_comp_fast(gt_array)
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    # load images
    print(reference_file, flush=True)
    ...
    for r in labels_or_regions:
        #gt_array, voxel_vol = nii2numpy(reference_file)
        #pred_array, voxel_vol = nii2numpy(prediction_file)
        results['metrics'][r] = {}

        mask_ref = region_or_label_to_mask(seg_ref, r)
        mask_pred = region_or_label_to_mask(seg_pred, r)
        tp, fp, fn, tn = compute_tp_fp_fn_tn(mask_ref, mask_pred, ignore_mask)
        #Modified by Bashirul
        if tp + fn == 0:
        ...
        #results['metrics'][r]['FNVol'] = false_neg_pix(gt_array, pred_array, reference_file)*voxel_vol
        #results['metrics'][r]['FPVol'] = false_pos_pix(gt_array, pred_array, reference_file)*voxel_vol
