...
class autoPET3_Trainer(nnUNetTrainer):
    ...
    def generate_mask_from_folder(self, data, case_identifier=None):
        """
        Generate or load masks from the specified mask folder.
        This method can be extended to include a pre-trained model for mask generation.
        
        Args:
            data: input image tensor [B, C, ...] on device
            case_identifier: optional identifier for saving/loading masks
            
        Returns:
            generated_mask: tensor with same spatial dimensions as data
        """
        if self.mask_generation_folder is None:
            return None
            
        # Option 1: Load pre-generated masks from folder
        if case_identifier is not None and os.path.exists(join(self.mask_generation_folder, f"{case_identifier}.npy")):
            mask = np.load(join(self.mask_generation_folder, f"{case_identifier}.npy"))
            mask = torch.from_numpy(mask).to(data.device, dtype=torch.float32)
            return mask
        
        # Option 2: Generate masks on-the-fly using a model
        # This is where you would add your mask generation logic
        # For example, using a pre-trained segmentation model:
        with torch.no_grad():
            # Example: simple thresholding or more complex generation
            # Replace this with your actual mask generation logic
            generated_mask = self._generate_mask_logic(data)
            
        # Optionally save the generated mask
        if case_identifier is not None:
            np.save(join(self.mask_generation_folder, f"{case_identifier}.npy"), 
                   generated_mask.cpu().numpy())
        
        return generated_mask
    
    def _generate_mask_logic(self, data):
        """
        Placeholder for actual mask generation logic.
        Override this method with your specific mask generation approach.
        ...
        """
        # Placeholder: You should implement your actual mask generation here
        # This could involve:
        # - Loading a pre-trained model
        # - Running inference
        # - Post-processing the predictions
        
        # For now, returning a simple threshold-based mask as example
        # Replace this with your actual implementation
        batch_size = data.shape[0]
        spatial_dims = data.shape[2:]
        
        # Example: create a simple mask (you should replace this)
        mask = torch.zeros((batch_size, 1) + spatial_dims, device=data.device, dtype=torch.float32)
        
        return mask


    def train_step(self, batch: dict, unlabelled_batch: dict = None) -> dict:
        data = batch['data']
        target = batch['target']
        #uncer = batch['uncer']
        #prob = batch['prob']
