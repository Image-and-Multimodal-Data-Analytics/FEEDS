...
Automated lesion segmentation in whole-body PET/CT can support cancer detection, staging, and treatment planning across radiotracers and cancer types — an increasingly important capability as imaging volume grows faster than the supply of nuclear medicine specialists. Training such models, however, requires large expert-annotated datasets, and annotation is time-, expertise-, and cost-intensive, made harder by the wide variability in lesion size, distribution, and appearance.

**FEEDS** addresses this bottleneck. It uses vision foundation model embeddings to select the most informative and diverse *unlabeled* cases for expert annotation *before any model training*. Unlike unsupervised, semi-supervised, and active learning approaches, FEEDS is a **one-step training paradigm** — a single upfront case selection followed by a single supervised training run — making it both label- and compute-efficient.

**Key result:** FEEDS matches fully-labeled (100%) training performance with a **30% labeling budget — a 70% reduction in annotation burden** — while outperforming random-sampling-based labeling, pseudolabel-based semi-supervised learning, and limited-labeled-data training. It generalizes across three held-out test sets, across FDG and PSMA tracers, and across multiple diseases.
